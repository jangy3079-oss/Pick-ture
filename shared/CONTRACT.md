...
## 0-2. `/problem.md` — 상하위 에이전트 공용 이슈 로그 & 다음 루프 반영 (반복 4, 실전에서 검증된 패턴을 계약으로 공식화)

`TASKS_FOR_AGENT2.md`/`AGENT2_STATUS.md`가 지시·보고용 단방향 채널이라면, `/problem.md`는 상위(Agent 1)와 하위(Agent 2)가 함께 쓰는 **양방향 이슈 로그**다.

- **쓰기 — 언제**: 다음을 발견하면 즉시 append한다 — ① CONTRACT.md/PLAN.md가 전제한 것과 실제 환경이 다를 때, ② 되돌리기 어렵거나 중대한 가정을 내렸을 때, ③ 사람이 방에 돌아왔을 때 반드시 알아야 할 문제(예: 무관한 프로세스를 건드림, 포트 변경), ④ 상대 에이전트 산출물에서 발견한 버그의 근본 원인 진단(코드는 고치지 않고 진단만 기록 — 수정 지시는 `TASKS_FOR_AGENT2.md`로).
- **쓰기 — 어떻게**: 항상 **append만 한다(전체 덮어쓰기 금지)**. 두 에이전트가 동시에 파일 전체를 새로 쓰면 레이스 컨디션으로 내용이 통째로 유실될 수 있음(실제로 반복 3~4 사이에 한 번 발생, 마지막 커밋에서 복구함).
- **읽기 — 언제**: 매 반복 시작 시, 각자의 상태 파일(Agent 1은 `AGENT2_STATUS.md`, Agent 2는 `TASKS_FOR_AGENT2.md`)과 **함께** `problem.md`도 다시 읽는다. 거기 기록된 내용을 그 반복의 작업 계획에 반영한다 — 예: 기록된 버그의 수정을 최우선 작업으로 올리거나, 기록된 가정과 충돌하지 않게 작업 범위를 조정한다.
- **사람을 위한 단일 진입점**: 사람이 방을 비운 동안 두 에이전트가 스스로 판단·가정하며 진행한 내용 중 사람이 검토해야 할 것들이 여기 모인다. 개별 STATUS 파일보다 `problem.md`를 먼저 보면 전체 그림이 보이게 유지한다.
