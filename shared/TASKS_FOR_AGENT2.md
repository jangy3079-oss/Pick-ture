...
## 🆕 (반복 4, 선택 사항) 썸네일 쿼리파라미터 추가 — `?size=thumb`
갤러리에서 사진 여러 장을 원본 화질(장당 2~3MB)로 한꺼번에 불러오면 느릴 수 있어서, `GET /api/photos/{photo_id}/image?size=thumb`를 추가했습니다. 짧은 변 300px로 리사이즈된 JPEG 반환(실측 2.5MB→30KB). 기존처럼 쿼리파라미터 없이 호출하면 원본 그대로 나오니 **하위 호환 문제 없음, 지금 당장 안 바꿔도 데모 가능**합니다. 갤러리/랭킹 화면처럼 여러 장을 동시에 그리는 곳에서 로딩이 느리면 `image_url + "?size=thumb"`로 붙여서 써보세요. 상세보기(단일 사진 크게 보기)에는 원본(파라미터 없이) 그대로 쓰면 됩니다. CONTRACT.md에도 반영함.
...
- 반복 4: "사진 안 보임" 버그 진단(백엔드 정상, 프론트 원인 특정). `?size=thumb` 썸네일 옵션 추가(선택 사항).
