# Pick!ture — 여행 사진 베스트컷 AI

> 📎 이 해커톤(원 펀치 랄프톤 in Busan)을 준비하며 **루프코딩(Loop Coding)**을 공부하기 위해 정리했던 자료 4편(루프코딩 연구실 가이드 / 루프코딩 해커톤 팀 적용 가이드 / 통합 트러블슈팅 & 토큰 효율 / 루프룸 방식 규칙 변경 대응판)은 [`document/`](document/) 폴더에 넣어두었습니다. 아래 내용은 그 자료를 전부 읽고, 루프코딩의 핵심 원칙 — *"목표를 검사 가능한 완료 조건으로 바꾸고, 공유 계약(CONTRACT.md)으로 병합 지옥을 막고, 루프는 대화가 아니라 디스크(파일)를 기준으로 매 반복을 새로 시작한다"* — 를 이해했다는 가정한 상태로 작성한 글입니다.

## 개발 주제 : 여행과 관련된 기발한 서비스 제작

**Pick!ture**는 여행 사진을 업로드하면 흔들린 사진과 중복 사진을 걸러내고, 여행지/국가별로 자동 앨범을 나누고, 사진마다 미학 점수를 매겨 베스트컷을 추려주고, 통계 리포트와 함께 블로그/인스타그램용 여행 후기 글까지 AI가 초안을 써주는 서비스입니다.
...
여행에서 돌아오면 보통 수백 장의 사진이 쌓입니다. 그중 실제로 SNS에 올리거나 앨범에 남길 만한 "베스트컷"은 소수이고, 나머지는 흔들렸거나, 같은 장면을 여러 번 찍었거나, 정리가 안 된 채로 방치됩니다. **Pick!ture**(Pick + Picture)는 그 정리 과정을 자동화하는 것이 목표입니다.
...
- 세션 도중 **Agent 2가 토큰을 소진**해 더 이상 작업할 수 없게 되었고, 해당 문제 상황을 Agent2가 problem.md에 문제상황을 작성, 이후 "상호 worktree 수정 금지" 규칙을 그 시점부터 해제해서 Agent 1이 프론트엔드까지 이어받아 완성했습니다.
...
├── document/                        # 해커톤 준비 중 정리한 루프코딩 학습 자료 4편
