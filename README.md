...
> 📎 이 해커톤(원 펀치 랄프톤 in Busan)을 준비하며 **루프코딩(Loop Coding)**을 공부하기 위해 정리했던 자료 3편(기본 가이드 / 해커톤 팀 적용 가이드 / 통합 트러블슈팅 & 토큰 효율)은 [`document/`](document/) 폴더에 넣어두었습니다. 아래 내용은 그 자료를 전부 읽고, 루프코딩의 핵심 원칙 — *"목표를 검사 가능한 완료 조건으로 바꾸고, 공유 계약(CONTRACT.md)으로 병합 지옥을 막고, 루프는 대화가 아니라 디스크(파일)를 기준으로 매 반복을 새로 시작한다"* — 를 이해한 상태로 작성했습니다.
...
├── document/                        # 해커톤 준비 중 정리한 루프코딩 학습 자료 3편
